...
void Sequence::prepareInterface() {
	// Register streams.
	registerStream("out_img0", &out_img0);
...
void Sequence::onLoadImage() {
	...
	try {
		...
		if (index == previous_index) {
			...
			if(out_switch == 1)	out_img0.write(img);
			if(out_switch == 2)	out_img1.write(img);
			return;
		}//: if
